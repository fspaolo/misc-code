...
class Mask(object):
    ...
    """
    def __init__(self, maskfile=None, x='x', y='y', mask='mask'):
        if maskfile is not None:
            self.getmask(maskfile, x, y, mask)    # HDF5 file
        else:
            self.maskfile = None 

    def __str__(self):
        return '\nMOA-/Scripps Antarctic mask of 1km resolution: %s\n' % self.maskfile

    def getmask(self, maskfile, x='x', y='y', mask='mask'):
