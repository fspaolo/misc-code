...
class GetData(object):
    def __init__(self, fname, mode='r'):
        fin = tb.open_file(fname, mode)
        self.file = fin
        try:
            self.satname = fin.get_node('/satname')[:]
        except:
            self.satname = fname.split('_')[0].split('/')[-1]
        try:
            self.time1 = fin.get_node('/time1')[:]
            self.time2 = fin.get_node('/time2')[:]
        except:
            self.time = fin.get_node('/time')[:]
        self.lon = fin.get_node('/lon')[:]
        self.lat = fin.get_node('/lat')[:]
        self.x_edges = fin.get_node('/x_edges')[:]
        self.y_edges = fin.get_node('/y_edges')[:]
        self.dh_mean = fin.get_node('/dh_mean')[:]
        self.dh_error = fin.get_node('/dh_error')[:]
        self.dh_error2 = fin.get_node('/dh_error2')[:]
        self.dg_mean = fin.get_node('/dg_mean')[:]
        self.dg_error = fin.get_node('/dg_error')[:]
        self.dg_error2 = fin.get_node('/dg_error2')[:]
        self.n_ad = fin.get_node('/n_ad')[:]
        self.n_da = fin.get_node('/n_da')[:]
        try:
            self.dh_mean_mixed_const = fin.get_node('/dh_mean_mixed_const')[:]
        except:
            pass
        try:
            self.dh_mean_short_const = fin.get_node('/dh_mean_short_const')[:]
        except:
            pass
...
class OutputContainers(object):
    def __init__(self, fname_out, shape, chunk):
        ...
        title = ''
        self.file = fout
        self.satname = fout.create_carray('/', 'satname', atom2, (nt,), 
            title, filters)
        self.time = fout.create_carray('/', 'time', atom, (nt,), 
            title, filters)
        self.lon = fout.create_carray('/', 'lon', atom, (nx,), 
            title, filters)
        self.lat = fout.create_carray('/', 'lat', atom, (ny,), 
            title, filters)
        self.x_edges = fout.create_carray('/', 'x_edges', atom, (nx+1,), 
            title, filters)
        self.y_edges = fout.create_carray('/', 'y_edges', atom, (ny+1,), 
            title, filters)
        self.dh_mean = fout.create_carray('/', 'dh_mean', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.dh_error = fout.create_carray('/', 'dh_error', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.dh_error2 = fout.create_carray('/', 'dh_error2', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.dg_mean = fout.create_carray('/', 'dg_mean', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.dg_error = fout.create_carray('/', 'dg_error', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.dg_error2 = fout.create_carray('/', 'dg_error2', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        self.n_ad = fout.create_carray('/', 'n_ad', atom, (nt,ny,nx), 
            title, filters, chunkshape=chunkshape)
        ...
        self.dh_mean_mixed_const = fout.create_carray('/', 
                ...
                title, filters, chunkshape=chunkshape)
        self.dh_mean_short_const = fout.create_carray('/', 
...
def get_data_from_file(fname_in, node_name, mode='r'):
    fin = tb.open_file(fname_in, mode)
    if node_name:
        data = fin.get_node('/', node_name)
    else:
        data = fin.root
    d = {}
    '''
...
def create_output_containers(fname_out, any_data, shape, node_name, chunkshape):
    ...
    file_out = tb.open_file(fname_out, 'w')
    ...
    dout = {}
    if node_name:
        g = file_out.create_group('/', node_name)
    else:
        g = '/' 
    dout['table'] = file_out.create_table(g, 'table', 
                                 TimeSeriesGrid, title, filters)
    dout['lon'] = file_out.create_carray(g, 'lon', 
                                  atom, (nx,), '', filters)
    dout['lat'] = file_out.create_carray(g, 'lat', 
                                  atom, (ny,), '', filters)
    dout['x_edges'] = file_out.create_carray(g, 'x_edges', 
                                  atom, (nx+1,), '', filters)
    dout['y_edges'] = file_out.create_carray(g, 'y_edges', 
                                  atom, (ny+1,), '', filters)
    dout['dh_mean'] = file_out.create_carray(g, 'dh_mean', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['dh_error'] = file_out.create_carray(g, 'dh_error', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['dh_error2'] = file_out.create_carray(g, 'dh_error2', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['dg_mean'] = file_out.create_carray(g, 'dg_mean', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['dg_error'] = file_out.create_carray(g, 'dg_error', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['dg_error2'] = file_out.create_carray(g, 'dg_error2', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['n_ad'] = file_out.create_carray(g, 'n_ad', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
    dout['n_da'] = file_out.create_carray(g, 'n_da', 
                                  atom, (N,ny,nx), '', filters, chunkshape=cs)
