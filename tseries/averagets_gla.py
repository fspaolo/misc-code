...
parser.add_argument('-n', dest='nodename', default='dh_mean',
    help='name of the node to average, default dh_mean')
...
def main(args):
    ...
    d, fin = get_data_from_file(fname_in, node_name=dh_name)
    _, ny, nx = d['dh_mean'].shape    # ni,nj,nk = nt,ny,nx

    print 'processing time series ...'
    print 'node_name:', dh_name
    ...
    for i in xrange(ny):
        for j in xrange(nx):
            ...
            if isfirst:
                ...
                time1.fill(time2[0])
                #time2 = [dt.time2 for dt in dh_mean_i.index]
                #month = [dt.month for dt in dh_mean_i.index]
                #time2 = ym_to_fy(time2, month)
