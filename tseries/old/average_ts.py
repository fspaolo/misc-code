...
$ python timeseries.py ~/data/fris/xover/seasonal/tmp/ers2_19950911_20030606_t.h5
"""
...
def main(argv):
    ...
    # plot time series
    fig = plt.figure()
    #d1, d2 = pn.datetime(1995, 9, 15), pn.datetime(2003, 7, 15)
    ...
    #plt.xlim(d1, d2)
    plt.ylabel('Elevation change (m)')
    t = add_inner_title(ax1, 'Multi-reference time (single grid cell)', loc=3)
    t = add_inner_title(ax2, 'Align + Average (single grid cell)', loc=3)
    fig.autofmt_xdate()
    ...
    for ts_i, se_i in zip(dh_ij_orig, se_ij_orig):
        ax1.plot(dates2, ts_i, linewidth=2)
        #ax1.errorbar(dates2, ts_i, yerr=se_i, linewidth=2)
    for ts in dh_ij:
        ax2.plot(dates2, ts, linewidth=2)
    ax2.plot(dates2, dh_j, 'w', linewidth=5)
    ax2.plot(dates2, dh_j, 'k', linewidth=3)

    #plt.savefig('ts_multi.pdf', dpi=150, bbox_inches='tight') 
    #os.system('cp ts_multi.pdf ~/posters/agu2011/figures/')
