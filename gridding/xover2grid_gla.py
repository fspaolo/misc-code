...
ABS_VAL = 3           # (m), accept data if |dh| < ABS_VAL
...
def main(args):
    ...
    for pos, fname in enumerate(files):
        ...
        d = apply_campaign_bias(d, source)  # substract biases
    ...
    try:
        print_info(x_edges, y_edges, lon, lat, dx, dy, n, source)
    except:
        print_info(x_edges, y_edges, lon, lat, dx, dy, 1, source)
    ...
    if PLOT:
        ...
        '''
        print np.sum(g.n_ad[~np.isnan(g.n_ad)]), np.sum(g.n_da[~np.isnan(g.n_da)])
