...
err = data['dhdt_poly_err(cm/yr)'].apply(np.abs).apply(np.round, decimals=1)
name = var.index
pos = np.arange(len(var))
...
#add the numbers to the side of each bar
for v, e, p in zip(var, err, pos):
    #plt.annotate(str(v)+' $\pm$ '+str(e), xy=(v + .7, p + .3), va='center')
    plt.annotate(str(v)+' $\pm$ '+str(e), xy=(v + .7, p + .3), va='center')
...
#plt.savefig('barchart.pdf', dpi=150)
plt.show()
