...
Chicago         -87.68  41.84
Houston         -95.39  29.77
New_York        -73.94  40.67
Vancouver       -123.13 49.28
Los_Angeles     -118.41 34.11
San_Francisco   -122.45 37.77
Atlanta         -84.42  33.76
Dubai           55.33   25.27
Sydney          151.21  -33.87
Hong_Kong       114.19  22.38
Bombay          72.82   18.96
Delhi           77.21   28.67
Newark          -82.43  40.04
Johannesburg    28.04   -26.19
Doha            51.53   25.29
Sao_Paulo       -46.63  -23.53
"""

###############################################################################
# Load the data, and put it in data structures we can use
routes_table = routes_data.split('\n')[1:-1]
cities_table = cities_data.split('\n')[1:-1]
routes_table = [s.split() for s in routes_table]
cities_table = [s.split() for s in cities_table]

# Build a dictionnary returning GPS coordinates for each city
cities_coord = dict()
for line in cities_table:
    name, long, lat = line
...
coords = list()
connections = list()
for city1, city2 in routes_table:
