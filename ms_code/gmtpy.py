...
class GMT:
    ...
    def _gmtcommand(self, command, 
        ...
        assert( 1 >= len([ x for x in [in_stream, 
		                               in_filename, 
									   in_string, 
									   in_columns, 
									   in_rows] if x is not None ]) )
        assert( 1 >= len([ x for x in [out_stream, 
		                               out_filename, 
									   out_discard] if x is not None ]) )
        ...
        for k,v in kwargs.items():
            if len(k) > 1:
                raise Exception('Found illegal keyword argument "%s" while \
				                 preparing options for command "%s"' % (k, command))
