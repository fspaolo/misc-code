#!/usr/bin/env python
"""
 Takes a command line statement and build (and submit) a batch file.
...
parser.add_argument('file', nargs='+', 
    help='file[s] to process [ex: /path/to/files/*.ext]')
parser.add_argument('-j', dest='jobname', default='job', 
    help='a symbolic name of the job [default: job]')
parser.add_argument('-o', dest='batchfile', default=None, 
    help='name of output batch file [default: <jobname>.xml]')
parser.add_argument('-e', dest='email', default='', 
    help="email to send notification of job state")
parser.add_argument('-s', dest='submit', default=False, action='store_const', 
    const=True, help='submit batch file after creation')
parser.add_argument('-c', dest='execute', 
    help="command (w/args) to execute [ex: -c '/path/to/prog -a arg']")

#parser.add_argument('-r', dest='jobid', default=None,
#    help="retrive results, only use this AFTER job is finished!")
...
#jobid = args.jobid
if not batchfile: batchfile = jobname+'.xml'
