...
def bindata(sample, bins=10, range=None, normed=False, weights=None):
    """
    Compute the multidimensional histogram of some data.
    ...
    # will raise an error.
    hist = zeros(nbin, float).reshape(-1)
    binned = zeros(nbin, float).reshape(-1)  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< me
    ...
    a = arange(len(flatcount))
    hist[a] = flatcount
    binned[a] = np.random.randn(len(flatcount))  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< me

    # main computation --------------------------------------------------------------

    # Shape into a proper matrix
    hist = hist.reshape(sort(nbin))
    binned = binned.reshape(sort(nbin))  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< me
    for i in arange(nbin.size):
        j = ni.argsort()[i]
        hist = hist.swapaxes(i,j)
        binned = binned.swapaxes(i,j)  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< me
        ni[i],ni[j] = ni[j],ni[i]
    ...
    if (hist.shape != nbin - 2).any() or (binned.shape != nbin - 2).any() :
        raise RuntimeError(
                "Internal Shape Error")
    return hist, edges, binned
